...
export async function generateMetadata({
  ...
  const pageUrl = `${appUrl}/${user.username}/locations/${save.location.id}`;
  const ogImage = save.location.photos[0]?.imagekitFilePath
    ? getPhotoUrl(save.location.photos[0].imagekitFilePath, "og")
    : undefined;
  // Always lead with the address so it survives even when a caption is set
  const description =
    [save.location.address, save.caption].filter(Boolean).join(" — ") ||
...
export default async function PublicLocationPage({
  ...
  // Public saves are viewable by anyone with the link, logged in or not
  ...
  const staticMapUrl = !primaryPhoto
    ? getStaticMapUrl(save.location.lat, save.location.lng)
    : null;
      ...
      <div className="relative aspect-video w-full">
        ...
        {save.isFavorite && (
          <div className="absolute top-4 right-4">
            <Badge
              variant="secondary"
              className="bg-warning/90 text-white border-0"
            >
      ...
      <div className="p-6 md:p-8">
          ...
          <div className="mb-6 p-4 bg-muted/50 rounded-lg border">
            ...
            {save.location.contactPhone && (
              <div className="text-sm">
                ...
                <a
                  href={`tel:${save.location.contactPhone}`}
        ...
        {(save.location.permitRequired || save.location.permitCost) && (
          <div className="mb-6 p-4 bg-muted/50 rounded-lg border">
            ...
            {save.location.permitCost !== null &&
              save.location.permitCost !== undefined && (
                <div className="text-sm flex items-center gap-1">
      ...
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3">
          <Button asChild variant="outline" size="lg" className="flex-1">
            <Link href={`/${user.username}`}>View {displayName}'s Profile</Link>
          </Button>
