...
const INBOUND_FORWARD_RECIPIENTS = (
  env.RESEND_INBOUND_FORWARD_TO ||
  process.env.RESEND_INBOUND_FORWARD_TO ||
  ""
)
  .split(",")
...
const INBOUND_FORWARD_FROM =
  env.RESEND_INBOUND_FORWARD_FROM ||
  process.env.RESEND_INBOUND_FORWARD_FROM ||
...
const EMAIL_EVENT_STATUS_MAP: Partial<
  Record<WebhookEventPayload["type"], string>
> = {
  "email.sent": "sent",
...
function parseSenderAddress(fromRaw: string): {
...
function getEventMetadataLine(
  ...
  if (event.type === "email.bounced" && data.bounce) {
    if (data.bounce.message) {
      metadataParts.push(
        `bounceMessage=${sanitizeMetadataValue(data.bounce.message)}`,
      );
    }
    if (data.bounce.type) {
  ...
  if (event.type === "email.failed" && data.failed?.reason) {
    metadataParts.push(
      `failureReason=${sanitizeMetadataValue(data.failed.reason)}`,
    );
  }
  ...
  if (event.type === "email.suppressed" && data.suppressed) {
    if (data.suppressed.message) {
      metadataParts.push(
        `suppressedMessage=${sanitizeMetadataValue(data.suppressed.message)}`,
      );
    }
...
async function maybeForwardInboundEmail(
  ...
  if (!emailId) {
    return {
      ...
      metadata: [
        ...
        "forwardError=missing inbound email_id",
      ],
  ...
  const to =
    INBOUND_FORWARD_RECIPIENTS.length === 1
      ? INBOUND_FORWARD_RECIPIENTS[0]
      : INBOUND_FORWARD_RECIPIENTS;
...
async function persistInboundEmail(
  ...
  if (!providerEmailId) {
    return [
      "inboundStored=false",
      "inboundStoreError=missing provider email_id",
    ];
  }
  ...
  const commonData = {
    ...
    rawExpiresAt: inboundEmail.raw?.expires_at
      ? parseEventDate(inboundEmail.raw.expires_at)
      : null,
    forwardConfigured: forwardResult.configured,
    forwardStatus: forwardResult.configured
...
export async function POST(request: NextRequest) {
  ...
  const webhookSecret =
    env.RESEND_WEBHOOK_SECRET || process.env.RESEND_WEBHOOK_SECRET;
  if (!webhookSecret) {
    return apiError(
  ...
  if (!messageId) {
    return apiError(
      "Email event missing message id",
      400,
      "INVALID_EMAIL_EVENT",
    );
  }
  ...
  const inboundPersistenceMetadata = await persistInboundEmail(
    resend,
    event,
    inboundForward,
  );
