...
import { generateToken, JWT_EXPIRY_SECONDS } from "@/lib/auth";

/**
 * How long a superseded iOS session stays valid after a new one is issued.
 * Rotation must not invalidate requests that are already in flight.
 */
const SESSION_ROTATION_GRACE_MS = 60 * 1000;

/**
 * Supersede the user's existing native iOS sessions without breaking in-flight
 * requests: shorten them to the grace window instead of deleting them outright.
 * Browser sessions on mobile devices ('mobile-browser-*') are left untouched.
 */
async function supersedeIosSessions(userId: number): Promise<void> {
  const graceExpiresAt = new Date(Date.now() + SESSION_ROTATION_GRACE_MS);
...
export async function POST(request: NextRequest) {
  try {
    ...
    return apiError(
      "Token Exchange Failed - Function POST - oAuth Token",
      500,
      "SERVER_ERROR",
    );
  }
...
async function handleAuthorizationCodeGrant(
  ...
  try {
    ...
    const sessionExpiresAt = new Date(Date.now() + JWT_EXPIRY_SECONDS * 1000);
    ...
    await supersedeIosSessions(authCode.userId);
    ...
    return apiError(
      "Token Exchange Failed - handleAuthorizationCodeGrant",
      500,
      "SERVER_ERROR",
    );
  }
...
async function handleRefreshTokenGrant(
  ...
  try {
    ...
    await supersedeIosSessions(refreshTokenRecord.userId);
