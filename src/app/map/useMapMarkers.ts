...
export function useMapMarkers({
  ...
  const expandBounds = useCallback(
    (bounds: google.maps.LatLngBounds): MapBounds => {
  ...
  useEffect(() => {
    if (locationsData?.locations) {
      ...
      const publicMarkers: MarkerData[] =
        showPublicLocations && publicLocationsData?.locations
  ...
  const createTemporaryMarkerFromClick = useCallback(
    async (
      position: { lat: number; lng: number },
      map: google.maps.Map | null,
    ) => {
      ...
      try {
        ...
        if (response.results && response.results.length > 0) {
          ...
          if (streetNumber && route) {
            ...
            name =
              addressResult.formatted_address.split(",")[0] ||
              "Selected Location";
          }
  ...
  const createTemporaryMarkerFromSearch = useCallback(
    (place: LocationData): MarkerData => {
