...
function MapPageInner() {
  ...
  const [selectedPublicLocation, setSelectedPublicLocation] =
  ...
  const { map, handleMapLoad, panToWithOffset } = useMapNavigation({
    markers,
    showPublicLocations,
    expandBounds,
    setMapBounds,
  ...
  const handleViewPublicDetails = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/locations/${loc.id}/photos?limit=100`, {
        credentials: "include",
      });
      if (res.ok) {
  ...
  return (
    <div className="fixed inset-0 top-16 flex flex-col">
      ...
      <div className="flex-1 relative">
        ...
        <CustomMapControls map={map} rightPanelOpen={rightPanelOpen} />
        ...
        <MapControls
          ...
          savedLocationsCount={
            markers.filter((m) => !m.isTemporary && !m.isPublic).length
          }
        />
      ...
      <Sheet open={showDetailsSheet} onOpenChange={setShowDetailsSheet}>
        <SheetContent className="w-full sm:w-1/2 p-0" hideOverlay={true}>
          <SheetHeader>
            <VisuallyHidden>
              <SheetTitle>
                {locationToEdit?.data?.name ||
                  locationToEdit?.userSave?.location?.name ||
                  "Location Details"}
              </SheetTitle>
            </VisuallyHidden>
          </SheetHeader>
            ...
            {locationToEdit?.userSave?.location && locationToEdit?.position && (
              <LocationDetailPanel
                location={{
                  ...
                  type:
                    locationToEdit.data?.type ||
                    locationToEdit.userSave.location?.type ||
                    "",
                  ...
                  productionDate:
                    locationToEdit.userSave.location?.productionDate ?? null,
                  productionNotes:
                    locationToEdit.userSave.location?.productionNotes ?? null,
                  details: locationToEdit.userSave.location?.details ?? null,
                  entryPoint:
                    locationToEdit.userSave.location?.entryPoint ?? null,
                  parking: locationToEdit.userSave.location?.parking ?? null,
                  access: locationToEdit.userSave.location?.access ?? null,
                  indoorOutdoor:
                    locationToEdit.userSave.location?.indoorOutdoor ?? null,
                  isPermanent:
                    locationToEdit.userSave.location?.isPermanent ?? false,
                  photoUrls:
                    locationToEdit.userSave.location?.photoUrls ?? null,
                  permitRequired:
                    locationToEdit.userSave.location?.permitRequired ?? false,
                  permitCost:
                    locationToEdit.userSave.location?.permitCost ?? null,
                  contactPerson:
                    locationToEdit.userSave.location?.contactPerson ?? null,
                  contactPhone:
                    locationToEdit.userSave.location?.contactPhone ?? null,
                  operatingHours:
                    locationToEdit.userSave.location?.operatingHours ?? null,
                  restrictions:
                    locationToEdit.userSave.location?.restrictions ?? null,
                  bestTimeOfDay:
                    locationToEdit.userSave.location?.bestTimeOfDay ?? null,
                  lastModifiedBy:
                    locationToEdit.userSave.location?.lastModifiedBy ?? null,
                  lastModifiedAt:
                    locationToEdit.userSave.location?.lastModifiedAt ?? null,
                  createdAt:
                    locationToEdit.userSave.location?.createdAt || new Date(),
                  updatedAt:
                    locationToEdit.userSave.location?.updatedAt || new Date(),
      ...
      <Sheet
        open={showPublicDetailsSheet}
        onOpenChange={setShowPublicDetailsSheet}
      >
        <SheetContent
          className="w-full sm:w-1/2 p-0"
          hideOverlay={true}
            ...
            {selectedPublicLocation && (
              <LocationDetailPanel
                location={{
                  ...
                  lastModifiedBy: null,
                  lastModifiedAt: null,
                  createdAt: selectedPublicLocation.savedAt
                    ? new Date(selectedPublicLocation.savedAt)
                    : new Date(),
                  updatedAt: new Date(),
                  photos:
                    publicLocationPhotos.length > 0
                  ...
                  userSave: {
                    ...
                    savedAt: selectedPublicLocation.savedAt
                      ? new Date(selectedPublicLocation.savedAt)
                      : new Date(),
...
function MapPageContent() {
  ...
  return (
    <ProtectedRoute>
      <OnboardingProvider
        userOnboardingStatus={userOnboardingStatus ?? undefined}
      >
        <MapPageInner />
        <TermsModal />
        <WelcomeModal />
