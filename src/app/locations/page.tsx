...
function LocationsPageInner() {
  ...
  const { data: publicLocationsData, isLoading: isLoadingPublic } =
    usePublicLocations({
      enabled: showPublic,
      type: typeFilter !== "all" ? typeFilter : undefined,
    });
  ...
  const { data: friendsLocationsData, isLoading: isLoadingFriends } =
    useFriendsLocations({
      enabled: showFriends,
      type: typeFilter !== "all" ? typeFilter : undefined,
    });
    ...
    if (editId && !hasProcessedEdit.current && data?.locations) {
      ...
      if (locationToEdit) {
        ...
        setTimeout(() => {
          ...
          setIndoorOutdoor(
            (locationToEdit.indoorOutdoor as "indoor" | "outdoor") || "outdoor",
          );
  ...
  if (favoritesOnly) {
    filteredLocations = filteredLocations.filter(
      (loc) =>
        (loc as LocationWithSource).source === "user" &&
        loc.userSave?.isFavorite,
    );
  }
  ...
  filteredLocations = [...filteredLocations].sort((a, b) => {
    switch (sortBy) {
      case "recent":
        return (
          new Date(b.userSave?.savedAt || b.createdAt).getTime() -
          new Date(a.userSave?.savedAt || a.createdAt).getTime()
        );
      case "oldest":
        return (
          new Date(a.userSave?.savedAt || a.createdAt).getTime() -
      ...
      case "rating":
        return (
          (b.userSave?.personalRating || 0) - (a.userSave?.personalRating || 0)
        );
      default:
        return 0;
    }
    ...
    <div className="fixed inset-0 top-16 flex flex-col">
      ...
      <div className="shrink-0 bg-background border-b">
        <div className="container mx-auto px-4 py-3 max-w-7xl">
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div
              className="flex-1 min-w-0 md:flex-none md:w-1/2"
              data-tour="locations-search"
            >
            ...
            <TooltipProvider delayDuration={300}>
              <div
                ...
                data-tour="location-source-toggles"
              >
                ...
                <Tooltip>
                  ...
                  <TooltipContent
                    side="bottom"
                    className="bg-foreground text-background border-border px-3 py-2 text-sm font-medium shadow-xl"
                  >
                    Show My Spots
                  </TooltipContent>
                </Tooltip>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={
                        showPublic
                          ? "bg-social hover:bg-social/90 text-white"
                          : ""
                      }
                    >
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="container mx-auto px-4 py-6 max-w-7xl">
          ...
          {showPublic &&
            !isLoadingPublic &&
            publicLocationsData?.locations.length === 0 && (
              <div className="bg-social/10 border border-social/20 rounded-lg p-6 mb-4">
                <h3 className="font-semibold text-social mb-2">
                  No public locations found
                </h3>
                <p className="text-sm text-social">
                  Try adjusting your filters or check back later for new public
                  locations.
                </p>
              </div>
            )}
          ...
          {showFriends &&
            !isLoadingFriends &&
            friendsLocationsData?.locations.length === 0 && (
              <div className="bg-primary/10 border border-primary/20 rounded-lg p-6 mb-4">
                <h3 className="font-semibold text-primary mb-2">
                  No friends&apos; locations found
                </h3>
                <p className="text-sm text-primary mb-3">
                  You&apos;re not following anyone yet, or your friends
                  haven&apos;t shared any locations.
                </p>
                <Link
                  href="/search"
          ...
          {viewMode === "grid" ? (
            <LocationList
              ...
              onEdit={(location) => {
                ...
                setIndoorOutdoor(
                  (location.indoorOutdoor as "indoor" | "outdoor") || "outdoor",
                );
                setShowPhotoUpload(false);
                setShowEditPanel(true);
              }}
          ...
          <div className="flex-1 min-h-0 overflow-hidden">
            {selectedLocation && (
              <LocationDetailPanel
                ...
                source={
                  (selectedLocation as LocationWithSource).source || "user"
                }
                canEdit={
                  user?.isAdmin ||
                  user?.role === "staffer" ||
                ...
                onEdit={(location) => {
                  ...
                  setIsFavorite(location.userSave?.isFavorite || false);
                  setIndoorOutdoor(
                    (location.indoorOutdoor as "indoor" | "outdoor") ||
                      "outdoor",
                  );
                  setShowPhotoUpload(false);
                ...
                onViewOnMap={(location) => {
                  ...
                  router.push(
                    `/map?lat=${location.lat}&lng=${location.lng}&zoom=17&edit=${userSaveId}`,
                  );
                }}
...
export default function LocationsPage() {
  const { user } = useAuth();
  const [locationsOnboardingCompleted, setLocationsOnboardingCompleted] =
    useState(false);
  ...
  useEffect(() => {
    if (user?.id) {
      fetch("/api/v1/users/me", {
        ...
        .then((data) => {
          if (data.user) {
            setLocationsOnboardingCompleted(
              data.user.locationsOnboardingCompleted ?? false,
            );
          }
        })
        .catch((err) =>
