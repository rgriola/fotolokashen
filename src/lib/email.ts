...
function getResendClient(): Resend {
  ...
  if (!resendClient) {
    throw new Error(
      "Resend API key is not configured. Set EMAIL_API_KEY in environment variables.",
    );
  }
  return resendClient;
}
...
export async function sendEmail(
  ...
  try {
    console.log(
      `[Email] Attempting send — mode: ${EMAIL_MODE}, to: ${to}, subject: "${subject}"`,
    );
    ...
    // The Resend SDK reports API failures via `error` rather than throwing.
    if (result.error) {
    ...
    console.log(
      `✅ Email sent to ${to}: ${subject} (Resend ID: ${resendId}, Reply-To: ${replyToAddress})`,
    );
    ...
    console.error(
      `❌ Email send FAILED — mode: ${EMAIL_MODE}, to: ${to}, subject: "${subject}"`,
    );
...
export async function sendEmailChangeVerification(
  ...
  if (EMAIL_MODE === "development") {
    ...
    console.log(`\nHi ${username},\n`);
    console.log(
      `You requested to change your email from ${oldEmail} to ${newEmail}.`,
    );
...
export async function sendEmailChangeConfirmation(
  ...
  if (EMAIL_MODE === "development") {
    console.log("\n" + "=".repeat(80));
    console.log(
      `✅ EMAIL CHANGE CONFIRMATION - ${emailType.toUpperCase()} (Development Mode)`,
    );
    console.log("=".repeat(80));
    console.log(`To: ${email}`);
