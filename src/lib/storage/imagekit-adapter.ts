 ...
 *
 * Concrete implementation of `StorageAdapter` (see ./types) backed by ImageKit.
 * This is the ONLY file in the codebase that should import the `imagekit` SDK
 * or know about ImageKit-specific transform syntax. Everything else should
 * import from `@/lib/storage` (see ./index.ts).
 */
 ...
 * SERVER-SIDE ONLY
 */
export async function deleteFromImageKit(fileId: string): Promise<StorageDeleteResult> {
    try {
        const imagekit = getImageKitInstance();
...
/**
 * Delete an entire folder (and its contents) from ImageKit.
 * SERVER-SIDE ONLY. Used for full cleanup of a user's files (e.g. account deletion).
 */
export async function deleteImageKitFolder(folderPath: string): Promise<StorageDeleteResult> {
    try {
        const imagekit = getImageKitInstance();
        await imagekit.deleteFolder(folderPath);
        return { success: true };
    } catch (error: unknown) {
        console.error('ImageKit folder delete error:', error);
        return {
            success: false,
...
/**
 * Generate raw ImageKit client-authentication parameters for direct
 * browser-to-CDN uploads (no folder/fileName binding).
 * SERVER-SIDE ONLY.
 */
export function getImageKitAuthParams(): StorageAuthParams {
    const imagekit = getImageKitInstance();
    const authParams = imagekit.getAuthenticationParameters();
...
/**
 * ImageKit implementation of the vendor-neutral `StorageAdapter` contract.
 * Prefer importing the named functions above for now (matches existing call
 * sites); use this object when code wants to depend on the `StorageAdapter`
 * interface directly (e.g. future vendor-swap tests).
 */
export const imagekitAdapter: StorageAdapter = {
    upload: (params) => uploadToImageKit(params),
    delete: (fileId) => deleteFromImageKit(fileId),
    deleteFolder: (folderPath) => deleteImageKitFolder(folderPath),
    getUrl: (filePath, transformations) => getImageKitUrl(filePath, transformations),
    getPhotoUrl: (filePath, variant) => getPhotoUrl(filePath, variant),
    getPhotoVariants: (filePath) => getPhotoVariants(filePath),
    generateSignedUploadUrl: (params) => generateSignedUploadUrl(params),
    getAuthParams: () => getImageKitAuthParams(),
};
