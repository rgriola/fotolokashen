...
export function EditLocationForm({
  ...
  const form = useForm<EditLocationFormData>({
    ...
    defaultValues: {
      ...
      type: location.type || "",
      indoorOutdoor:
        (location.indoorOutdoor as "indoor" | "outdoor" | "both") ||
        DEFAULT_INDOOR_OUTDOOR,
  ...
  const watchedProductionDate = useWatch({
    control: form.control,
    name: "productionDate",
  });
  const watchedProductionNotes = useWatch({
    control: form.control,
    name: "productionNotes",
  });
  const watchedPersonalRating = useWatch({
    control: form.control,
    name: "personalRating",
  });
  const watchedIndoorOutdoor = useWatch({
    control: form.control,
    name: "indoorOutdoor",
  });
  const watchedParking = useWatch({ control: form.control, name: "parking" });
  const watchedEntryPoint = useWatch({
    control: form.control,
    name: "entryPoint",
  });
  ...
  useEffect(() => {
    ...
    const photosChanged =
      JSON.stringify(originalPhotos) !== JSON.stringify(currentPhotos);
    if (photosChanged) {
  ...
  const handleSubmit = async (data: EditLocationFormData) => {
    ...
    if (cachedPhotos.length > 0 && uploadPhotosRef.current) {
      console.log(
        "[EditLocationForm] Starting photo upload for",
        cachedPhotos.length,
        "photos",
      );
      try {
        ...
        console.log(
          "[EditLocationForm] Photos uploaded successfully:",
          uploadedPhotos,
        );
  ...
  const finalizeSubmit = async (
    ...
    const remainingPhotos = allPhotos.filter(
      (photo) => !photo.id || !photosToDelete.includes(photo.id),
    );
  ...
  return (
    ...
    >
            ...
            <div className="flex items-center gap-2 mt-2">
              <Button
                ...
                disabled={
                  isAiLoading ||
                  !watchedProductionNotes ||
                  watchedProductionNotes.trim().length === 0
                }
      ...
      <AlertDialog
        open={showPhotoDeleteDialog}
        onOpenChange={setShowPhotoDeleteDialog}
      >
