...
export function CreateLocationWithPhoto({
  onSuccess,
}: CreateLocationWithPhotoProps) {
  ...
  const hasLocation = Boolean(
    (gpsData?.hasGPS && gpsData.lat && gpsData.lng) || manualLocation,
  );
  ...
  const canSave = Boolean(
    photoFile && hasLocation && form.watch("name") && form.watch("type"),
  );
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (metadata.hasGPS && metadata.lat && metadata.lng) {
          ...
          form.setValue(
            "placeId",
            address?.placeId || fallbackPlaceIdRef.current,
          );
          form.setValue("name", address?.name || "Photo Location");
          form.setValue(
            "address",
            address?.address ||
              `Location at ${metadata.lat.toFixed(6)}, ${metadata.lng.toFixed(6)}`,
          );
          ...
          console.log(
            "⚠️ No GPS in photo EXIF - enabling manual location mode",
          );
  ...
  const handleSubmit = async (data: CreateLocationFormData) => {
    ...
    try {
      ...
      console.log(
        "[CreateLocationWithPhoto] Uploading photo via secure endpoint...",
      );
      ...
      {error && (
        <div className="p-4 bg-destructive/10 dark:bg-destructive/10 border border-destructive/20 dark:border-destructive rounded-lg flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-destructive dark:text-destructive-foreground mb-1">
              Error
            </p>
                ...
                <div className="p-3 bg-muted rounded-lg text-xs space-y-1">
                  ...
                  {gpsData.dateTaken && (
                    <p>
                      ...
                      {new Date(gpsData.dateTaken).toLocaleString()}
                    </p>
                  )}
