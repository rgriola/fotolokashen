...
import { getColorForType } from "@/lib/map-icon-colors";
...
export function LocationsMapView({ locations }: LocationsMapViewProps) {
  ...
  useEffect(() => {
    const markerData: MarkerData[] = locations.map((location) => ({
      ...
      location,
      color:
        location.userSave?.color || getColorForType(location.type || "OTHER"),
    }));
          ...
          >
            <div className="p-2 max-w-xs">
              ...
              {selectedMarker.location.userSave?.personalRating ? (
                <div className="flex items-center gap-1 mb-2">
                  ...
                  <span className="text-muted-foreground">
                    {"★".repeat(
                      5 - selectedMarker.location.userSave.personalRating,
                    )}
                  </span>
                </div>
              ) : null}
              ...
              <p className="text-xs text-muted-foreground mb-3">
                {selectedMarker.position.lat.toFixed(3)},{" "}
                {selectedMarker.position.lng.toFixed(3)}
              </p>
