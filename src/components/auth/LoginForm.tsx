    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (result.requiresVerification && result.email) {
          // Captured so the narrowing survives into the setTimeout closures below.
          const verificationEmail = result.email;

          // Check if a new verification email was sent
          if (
            result.code === "EMAIL_NOT_VERIFIED_RESENT" ||
            result.tokenResent
          ) {
            ...
            setTimeout(() => {
              router.push(
                `/verify-email?email=${encodeURIComponent(verificationEmail)}&resent=true`,
              );
            }, 1000);
            return;
      ...
      if (
        ...
        try {
          ...
          if (!oauthResponse.ok) {
            setOauthDebug(
              `2/4 FAIL: ${oauthResponse.status} - ${JSON.stringify(oauthResult)}`,
            );
